    ...
    with alive_bar(len(metadata), title="Building links") as bar:
        for paper in metadata:
            for reference in paper["references"]:
                links.append(
                    {"source": paper["paperId"], "target": reference["paperId"]}
                )
            bar()
...
def build_graph(session, bibliography):
    ...
    bibliography_paper_ids = [
        x["paperId"] for x in get_metadata_mult(session, bibliography) if x
    ]
    ...
    node_metadata = get_metadata_mult(
        session,
    ...
    links = [
        x
        for x in links
        if x["source"] in [y["paperId"] for y in nodes]
        and x["target"] in [y["paperId"] for y in nodes]
    ]
